            ...
            <div class="container-fluid">
                ...
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">Mitra Penelitian</h3>
                    </div>
                    <!-- /.card-header -->
                    <!-- form start -->
                    <div class="card-body p-0">
                        <div class="bs-stepper">
                            ...
                            <form action="{{route('pengusul_penelitian_upload_dokumen_mitra', $id)}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PATCH')
                                <div class="row m-4">
                                    <div class="col-md-8">
                                        <div class="form-group">
                                            ...
                                            <h6>File Mitra Penelitian : @if($dokumen_info){{$dokumen_info->mitra_sasaran_file_original_name}}@else{{"-"}}@endif</h6>
                                            <h6>Tanggal Unggah : @if($dokumen_info){{Carbon\Carbon::parse($dokumen_info->updated_at)->isoFormat('D MMMM Y')}}@else{{"-"}}@endif</h6>
                                            <h6>Ukuran File : @if($dokumen_info){{$dokumen_info->mitra_sasaran_file_size . " KB"}}@else{{"-"}}@endif</h6>

                                            @if($dokumen_info)
                                            <a href="{{route('pengusul_penelitian_file_preview', [$id, $dokumen_info->mitra_sasaran_file_hash_name, 'mitra'])}}" class="ml-1 btn btn-sm btn-primary" target="__blank"><i class="fas fa-eye"></i> Preview</a>
                                            <a href="{{route('pengusul_penelitian_file_download', [$id, $dokumen_info->mitra_sasaran_file_hash_name, 'mitra'])}}" class="ml-1 btn btn-sm btn-success"><i class="fas fa-cloud-download-alt"></i> Download</a>
                                            <br>
                                            <br>
                                            @endif
                                            ...
                                            <div class="input-group  @error('dokumen_mitra') is-invalid @enderror">
                                                <div class="custom-file">
                                                    ...
                                                    <label class="custom-file-label" for="dokumen_mitra">Unggah File</label>
                                                </div>
